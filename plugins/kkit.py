            ...
            if filters[str(filter_)] == 'SBML':
                ...
                if written == False:
                    ...
                    if error == "":
                        ...
                        #status = PyQt5.QtCore.QString("File saved but %2").arg(error);
...
class  KineticsWidget(EditorWidgetBase):
    ...
    def updateModelView(self):
        ...
        if not self.m:
            ...
            #self.connect(self.objectEditSlot)
            ...
            if isinstance(self,kineticEditorWidget):
                ...
                self.view.setRefWidget("editorView")
                self.view.setAcceptDrops(True)
                #self.view.dropped.emit(self.objectEditSlot)
                self.view.dropped.connect(self.objectEditSlot)
                #self.view.dropped.connect(self.objectEditSlot)
    ...
    def updateItemSlot(self, mooseObject):
        ...
        for item in self.sceneContainer.items():
            if isinstance(item,PoolItem):
                #print(mooseObject,"## ",element(item.mobj))
                if mooseObject.id == element(item.mobj).id:
                    item.updateSlot()
    ...
    def updateColorSlot(self,mooseObject, colour):
        ...
        elif (mooseObject.isA("PoolBase") or mooseObject.isA("EnzBase") ):
    ...
    def mooseObjOntoscene(self):
        ...
        for k,v in self.objPar.items():
            if (moose.element(k).isA("ChemCompt")):
                self.createCompt(k)
                self.qGraCompt[k]
                
            elif (moose.element(k).isA("Neutral")):
                if len(self.meshEntry[k]):
                    if moose.element(v).isA("ChemCompt"):
                        group_parent = self.qGraCompt[v]
                    elif moose.element(v).isA("Neutral"):
                        group_parent = self.qGraGrp[v]
                    self.createGroup(k,group_parent)
        for cmpt_grp,memb in self.meshEntry.items():
            if len(memb):
                if (moose.element(cmpt_grp).isA("ChemCompt")):
                    qtGrpparent = self.qGraCompt[cmpt_grp]
                elif (moose.element(cmpt_grp).isA("Neutral")):
    ...
    def drawLine(self,srcdes_list,arrow):
        ...
        elif (source.isA("PoolBase") or source.isA("Function")):
            pen.setColor(QtCore.Qt.blue)
        elif (source.isA("StimulusTable")):
            ...
            for k,v in self.qGraGrp.items():
                for grpChilditem in v.childItems():
                    if isinstance(grpChilditem, KineticsDisplayItem):
                        ...
                        if (moose.element(grpChilditem.mobj.path).isA("PoolBase")):
    ...
    def positionChange1(self,mooseObject):
        #If the item position changes, the corresponding arrow's are calculated
        if ( (element(mooseObject).isA("CubeMesh")) or (element(mooseObject).isA("CylMesh"))):
