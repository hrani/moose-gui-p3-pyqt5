...
getxyCorf, etc function are added
'''
import collections
from moose import *
import numpy as np
from moose import wildcardFind,element,PoolBase,Annotator,exists
...
def updateMeshObj(modelRoot):
    ...
    for compt in wildcardFind(modelRoot+'/##[ISA=ChemCompt]'):
        for m in wildcardFind(compt.path+'/##[ISA=PoolBase]'):
            ...
            if (element(grp_cmpt).isA("Neutral")):
                if (element(m.parent).isA("EnzBase")):
        ...
        for r in wildcardFind(compt.path+'/##[ISA=Reac]'):
            rgrp_cmpt = findGroup_compt(r)
            if (element(rgrp_cmpt).isA("Neutral")):
        ...
        for e in wildcardFind(compt.path+'/##[ISA=EnzBase]'):
            ...
            if (element(egrp_cmpt).isA("Neutral")):
        ...
        for f in wildcardFind(compt.path+'/##[ISA=Function]'):
            ...
            if (element(fgrp_cmpt).isA("Neutral")):
        ...
        for t in wildcardFind(compt.path+'/##[ISA=StimulusTable]'):
            ...
            if (element(tgrp_cmpt).isA("Neutral")):
...
def setupMeshObj(modelRoot):
    ...
    for compt in wildcardFind(modelRoot+'/##[ISA=ChemCompt]'):
        for m in wildcardFind(compt.path+'/##[ISA=PoolBase]'):
            grp_cmpt = findGroup_compt(m)
            if not element(m.parent).isA("EnzBase"):
