...
class GraphicalView(QGraphicsView):
    ...
    def resizeEvent(self, event):
        print ("@@ kvc 89",event.size().width(),event.size().height())
        ...
        if clickedItemType == ITEM:
            ...
            if actionType == "delete":
                ...
                if reply == QMessageBox.Yes:
                    ...
                    deleteChemSolver(self.layoutPt.modelRoot)
                ...
                if element.isA("PoolBase"):
                    ...
                    self.pushButtonNumber = QPushButton(('Number'))#, QtGui.QMessageBox.YesRole)
                    self.pushButtonConc = QPushButton('Concentration')#, QtGui.QMessageBox.NoRole
                ...
                if self.state["move"]["happened"]:
                    ...
                    itemAtView = self.sceneContainerPt.itemAt(posWrtComp,QtGui.QTransform())
                    #itemAtView = self.sceneContainerPt.itemAt(self.mapToScene(event.pos().toPoint, QtGui.QTransform()))
                    self.removeConnector()
                    if isinstance(itemAtView,ComptItem) or isinstance(itemAtView, GRPItem):
                        ...
                        if isinstance (itemAtView, GRPItem):
                            lKey = [key for key, value in self.layoutPt.qGraGrp.items() if value == itemAtView][0]
                        iR = 0
                        iP = 0
                        t = moose.element(cloneObj.parent().mobj)
                        name = t.name
                        if (cloneObj.parent().mobj.isA("PoolBase")):
                        ...
                        if (cloneObj.parent().mobj.isA("Reac")):
    ...
    def onClicked(self,modelRoot,element, btn):
        ...
        reply = QMessageBox.information(self, "plot Object","Plot is added to Graph1",
                                      QMessageBox.Ok)
        self.removeConnector()
    ...
    def objectpullback(self,messgtype,item,movedGraphObj,xx,yy):
        ...
        QtWidgets.QMessageBox.warning(None,'Could not move the object', messgstr )
    ...
    def deleteGroup(self,item,layoutPt):
        reply = QMessageBox.question(self, "Deleting Object",'Do want to delete group \'{groupname}\' and its children and connections'.format(groupname=item.mobj.name),
                                                  QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
            deleteChemSolver(self.layoutPt.modelRoot)
    ...
    def showConnector(self, item):
        ...
        for l in self.connectorlist.keys():
            ...
            if moose.element(item.mobj).isA("PoolBase") or moose.element(item.mobj).isA("Reac"):
                if l == "clone":
            ...
            if moose.element(item.mobj).isA("PoolBase"):
                if l == "plot":
    ...
    def deleteConnection(self,item):
        ...
        reply = QtWidgets.QMessageBox.question(self, "Deleting Object","Do want to delete object and its connections",
        ...
        if isinstance(item,KineticsDisplayItem)  or isinstance(item,GRPItem):
            if moose.exists(item.mobj.path):
                if isinstance(item,PoolItem) or isinstance(item,BufPool):
                    ...
                    for items in moose.element(item.mobj.path).children:
                        ...
                        if ((items).isA("EnzBase")):
                            ...
                            moose.delete(moose.element(items))
                ...
                for key, value in self.layoutPt.object2line.items():
                    self.layoutPt.object2line[key] = list(filter( lambda tup: tup[1] != item ,value))
    ...
    def populate_srcdes(self,src,des):
        ...
        if (src.isA("PoolBase") and ((des.isA("Reac")) or (des.isA("EnzBase")))):
        #if ( isinstance(moose.element(src),Pool) and ( (isinstance(moose.element(des),Reac) ) or isinstance(moose.element(des),EnzBase) )):
            ...
            if foundp == False and founds == False:
                ...
                QtWidgets.QMessageBox.information(None,'Connection Not possible','{srcdesString}'.format(srcdesString = srcdesString),QtWidgets.QMessageBox.Ok)
            elif founds:
                ...
                QtWidgets.QMessageBox.information(None,'Connection Not possible','{srcdesString}'.format(srcdesString = srcdesString),QtWidgets.QMessageBox.Ok)    
        elif (src.isA("PoolBase") and (des.isA("Function"))):               
        ...
        elif ( src.isA("Function") and des.isA("PoolBase")):  
            if ((element(des).parent).className != 'Enz'):
                ...
                QtWidgets.QMessageBox.information(None,'Connection Not possible','\'{srcdesString}\' not allowed to connect'.format(srcdesString = srcdesString),QtWidgets.QMessageBox.Ok)
                callsetupItem = False
        elif ( src.isA("Function") and  des.isA("BufPool") ): #or  
            moose.connect(src, 'valueOut', des, 'setN', 'OneToOne')
        
        elif ( src.isA("Function") and  des.isA("Reac") ): #or  
            moose.connect(src, 'valueOut', des, 'setNumKf', 'OneToOne')
        
        elif ( ( src.isA("Reac") or src.isA("EnzBase") ) and des.isA("PoolBase") ):
            founds,foundp = False,False
        ...
        elif( src.isA("StimulusTable") and des.isA("PoolBase")  ):
