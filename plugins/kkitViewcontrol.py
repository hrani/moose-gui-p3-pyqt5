...
class GraphicalView(QGraphicsView):
        ...
        if clickedItemType == ITEM:
            ...
            if actionType == "delete":
                ...
                if element.isA("PoolBase"):
                    ...
                    self.pushButtonNumber = QPushButton(('Number'))#, QMessageBox.YesRole)
                    self.pushButtonConc = QPushButton('Concentration')#, QMessageBox.NoRole
    ...
    def populate_srcdes(self,src,des):
        ...
        if (  moose.element(src).isA("PoolBase") and ( moose.element(des).isA("Reac") or moose.element(des).isA("EnzBase") )):
        ...
        elif ( moose.element(src).isA("PoolBase") and moose.element(des).isA("Function")):
            ...
            moose.connect( src, 'nOut', des.x[numVariables], 'input' )
        ...
        elif ( moose.element(src).isA("Function") and moose.element(des).isA("Pool") 
            ):
                if ((element(des).parent).className != 'Enz'):
                    ...
                    QMessageBox.information(None,'Connection Not possible','\'{srcdesString}\' not allowed to connect'.format(srcdesString = srcdesString),QMessageBox.Ok)
                    callsetupItem = False
        elif ( moose.element(src).isA("Function") and (moose.element(des).className=="BufPool")
            ):
                moose.connect(src, 'valueOut', des, 'setN', 'OneToOne')
        elif ( moose.element(src).isA("Function") and (moose.element(des).isA("Reac") ) 
            ):
                moose.connect(src, 'valueOut', des, 'setNumKf', 'OneToOne')
        elif ( ( moose.element(src).isA("Reac") or moose.element(src).isA("EnzBase") ) and moose.element(des).isA("PoolBase") ):
            founds,foundp = False,False
            ...
            if founds == False and foundp == False:
                ...
                QMessageBox.information(None,'Connection Not possible','{srcdesString}'.format(srcdesString = srcdesString),QMessageBox.Ok)
        elif( moose.element(src).isA("StimulusTable") and (moose.element(des).isA("PoolBase") ) ):
